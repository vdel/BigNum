...
main (int argc, char *argv[])
{
  ...
  // Addition
  double add_per_sec_gmp;
  printf ("Calibrating CPU speed...");  fflush (stdout);
  TIME (t, mpz_add (z, xptr, yptr));
  printf ("done\n");

  niter = 1 + (unsigned long) (1e4 / t);
  
  if (argc == 2)
    printf ("Doubling a %lu-bit number %lu times with GMP...", m, niter);
  else
    printf ("Adding %lu-bit number with %lu-bit number %lu times with GMP...", m, n, niter);
  fflush (stdout);     
  t0 = cputime ();
  for (i = niter; i > 0; i--)
    mpz_add (z, xptr, yptr);
  ti = cputime () - t0;
  printf ("done!\n");
  add_per_sec_gmp = 1000.0 * niter / ti;
 
  double vm2, res2;
  process_mem_usage(vm2,res2);

  printf ("RESULTS:\n");
  printf("GMP: %f additions per second\n", add_per_sec_gmp);
