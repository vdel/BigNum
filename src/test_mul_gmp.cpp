...
main (int argc, char *argv[])
{
  ...
  if (argc == 2)
    printf ("Squaring a %lu-bit number %lu times with GMP...", m, niter);
  else
    printf ("Multiplying %lu-bit number with %lu-bit number %lu times with GMP...", m, n, niter);
  fflush (stdout);     
  t0 = cputime ();
  for (i = niter; i > 0; i--)
    mpz_mul (z, xptr, yptr);
  ti = cputime () - t0;
  printf ("done!\n");
  mul_per_sec_gmp = 1000.0 * niter / ti;
  
  double vm2, res2;
  process_mem_usage(vm2,res2);

  printf ("RESULTS:\n");
  printf("GMP: %f multiplication per second\n", mul_per_sec_gmp);
